#!/bin/sh
# Compile Walrus_I2C.cpp on the host against the stubs here and diff the output
...
g++ -std=c++17 -Wall -Wno-unused-function -I"$NW_CORE/extras/test" -I"$NW_CORE/src" -o walrus_test test_output.cpp "$NW_CORE/src/NW_Device.cpp" || exit 1
./walrus_test > output.txt || exit 1
